...
{#if isOpen}
  <div class="fixed inset-y-0 left-0 w-64 bg-card border-r border-border shadow-lg z-50 transform transition-transform duration-200 ease-in-out p-4 flex flex-col">
    <div class="flex items-center justify-between mb-6">
      <h2 class="font-playfair text-lg font-semibold text-foreground">My Strategies</h2>
      <button on:click={() => isOpen = false} class="text-muted-foreground hover:text-foreground">
    ...
    {:else}
      <div class="flex-1 overflow-y-auto space-y-2">
        {#each plans as p}
          ...
          >
            <div class="font-medium text-sm text-foreground group-hover:text-primary truncate">{p.title}</div>
            <div class="text-xs text-muted-foreground">{new Date(p.updated_at).toLocaleDateString()}</div>
          </button>
        {/each}
      </div>
    {/if}
    ...
    <div class="mt-4 pt-4 border-t border-border">
         <div class="text-xs text-muted-foreground text-center">LegalLens v0.1</div>
    </div>
  </div>
  
  <!-- Overlay -->
  <div class="fixed inset-0 bg-background/80 backdrop-blur-sm z-40" on:click={() => isOpen = false}></div>
{/if}
